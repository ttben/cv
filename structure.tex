...
\geometry{
	hmargin=1.25cm, % Horizontal margin
...
\setlength\columnsep{0.04\textwidth} % Specify the amount of space between the columns
...
\definecolor{shade}{HTML}{E7E7E7} % Peach colour for the contact information box
...
\noindent {\footnotesize #1} \hfill {\footnotesize #3}\hfill {\footnotesize(#2)}\\
%
% {\noindent\small #3\hfill #1 (#2)}\\ % Job title
...
% Command for entering a new course
\newcommand{\course}[4]{
\noindent {\footnotesize #1} \hfill {\footnotesize #2}\\
%
% {\noindent\small #3\hfill #1 (#2)}\\ % Job title
	% {\raggedleft\textsc{#1\expandafter\ifstrequal\expandafter{#2}{}{}{\hspace{6pt}\footnotesize{(#2)}}}\par} % Duration and conditional full time/part time text
	% \expandafter\ifstrequal\expandafter{#3}{}{}{{\raggedright\large #3}\\} % Employer
...
\newcommand{\educationentry}[5]{
	...
	\expandafter\ifstrequal\expandafter{#3}{}{}{& {\footnotesize\textsc{#3}}\\} % Honours, achievements or distinctions (e.g. first class honours)
	\expandafter\ifstrequal\expandafter{#4}{}{}{& #4\\} % Department
	\expandafter\ifstrequal\expandafter{#5}{}{}{& \small#5\\[6pt]} % Institution
}
...
\newcommand{\longformpublicationWithDOI}[5] {
	#1\\
	{\small#2}\\
	{\small#3}\\
	% \href{http://dx.doi.org/#4}{\expandafter\ifstrequal\expandafter{#5}{firstauthor}{\textbf{doi:#4}}{doi:#4}}\medskip% DOI string and if "firstauthor" is entered for the 3rd argument, this makes the DOI string bold indicating a first author publication
